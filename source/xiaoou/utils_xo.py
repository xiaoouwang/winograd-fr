...
def xo_produce_answers_js(model, pd_good):
    for i, row in pd_good.iterrows():
        ...
        options = [row.correct_answer_local_contexte, row.wrong_answer_local_contexte]
...
def xo_computer_score(pd_good):
    ...
    data = {"correct": [round(counter_correct, 2)], "no_response": [round(counter_noresponse, 2)]
        , "bad_response": [round(counter_badresponse, 2)]
        , "total_responses": [round(counter_total, 2)]
        , "exactitude": [round((counter_correct / counter_total) * 100, 2)]
        , "qualite": [round((counter_correct / (counter_total - counter_noresponse)) * 100, 2)]
        , "reussite": [round(((counter_correct + counter_noresponse / 2) / counter_total) * 100, 2)]}
